...
public:
	 ...
	 * constructor 
	 */	
	nccl_net_ofi_domain_t(nccl_net_ofi_device_t *device_arg, long creating_thread_id);
	 ...
	 * "leader NIC"
	 */
	virtual struct fid_domain *get_ofi_domain() = 0;
	 ...
	 * case, this will be the info object associated with the "leader NIC"
	 */
	virtual struct fid_cq *get_ofi_cq() = 0;
	 ...
	 */
	inline nccl_net_ofi_device_t *get_base_device()
	{
		return device;
	}

	/**
	 * @brief	Returns the base domain's endpoint pointer.
	 */
	...
	/**
	 * @brief	Set the base domain's endpoint pointer to nullptr.
	 */
	inline void clear_base_endpoint_ptr()
	{
		endpoint = nullptr;
	}
...
protected:
	 ...
	 * to make it safe to call in domain destructors. Set called_cleanup_resources to true
	 * at the start of the function to make sure it is only called once per domain
	 * instance.
	 * 
