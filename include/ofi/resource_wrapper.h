...
struct ofi_result {
    ...
    resource_ptr& operator*() { return resource; }
    const resource_ptr& operator*() const { return resource; }
    resource_ptr* operator->() { return &resource; }
    const resource_ptr* operator->() const { return &resource; }
};

// Type aliases for each resource type
using ofi_fabric_result = ofi_result<ofi_fabric_ptr>;
using ofi_domain_result = ofi_result<ofi_domain_ptr>;
using ofi_ep_result = ofi_result<ofi_ep_ptr>;
using ofi_av_result = ofi_result<ofi_av_ptr>;
using ofi_cq_result = ofi_result<ofi_cq_ptr>;
using ofi_mr_result = ofi_result<ofi_mr_ptr>;
