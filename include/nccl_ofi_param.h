 ...
 * so requesting it is always API-safe. This raw switch is resolved once at
 * init (together with the compile-time support and provider checks) into the
 * global nccl_ofi_use_relaxed_ordering during querying provider capabilities.
 */
