...
#define NCCL_OFI_RDMA_RECV_COMPLETION_OPTIONAL_BITS (1)
...
typedef struct nccl_net_ofi_ctrl_fifo {
	...
	uint8_t recv_completion_optional:NCCL_OFI_RDMA_RECV_COMPLETION_OPTIONAL_BITS;

	/* Control message sequence number */
	uint16_t msg_seq_num:NCCL_OFI_RDMA_SEQ_BITS;
...
static_assert(sizeof(nccl_net_ofi_ctrl_fifo_t) == 48,
              "Wrong size for RDMA Control message");
static_assert(offsetof(nccl_net_ofi_ctrl_fifo_t, short_buff_mr_key) +
	       sizeof( ((nccl_net_ofi_ctrl_fifo_t *)0)->short_buff_mr_key) <= 32,
	       "Short RDMA Control message larger than 32 bytes (EFA inline size)");

#define NCCL_NET_OFI_CTRL_MSG_SHORT_KEY_SIZE (sizeof( ((nccl_net_ofi_ctrl_fifo_t *)0)->short_buff_mr_key[0] ))
#define NCCL_NET_OFI_CTRL_MSG_LONG_KEY_SIZE (sizeof( ((nccl_net_ofi_ctrl_fifo_t *)0)->long_buff_mr_key[0] ))

/* Ctrl fifo size minus mr key */
#define NCCL_NET_OFI_CTRL_MSG_BASE_SIZE 14
...
static inline size_t nccl_net_ofi_rdma_ctrl_msg_size(uint16_t num_rails, bool use_long_rkeys)
{
	...
	return NCCL_NET_OFI_CTRL_MSG_BASE_SIZE + num_rails * rkey_len;
}
