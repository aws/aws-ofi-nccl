...
typedef struct nccl_net_ofi_ctrl_fifo {
	...
	uint16_t recv_completion_optional:NCCL_OFI_RDMA_RECV_COMPLETION_OPTIONAL_BITS;

	/* Control message sequence number */
	uint16_t msg_seq_num:NCCL_OFI_RDMA_SEQ_BITS;

	uint8_t page_size_padding[18];
...
static_assert(offsetof(nccl_net_ofi_ctrl_fifo_t, page_size_padding) == 46,
