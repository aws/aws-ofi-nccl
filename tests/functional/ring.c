...
int main(int argc, char *argv[])
{
	...
	if (size < 2) {
		NCCL_OFI_WARN(
			"Expected at least two ranks but got %d. "
	...
	for (int dev_idx = 0; dev_idx < ndev; dev_idx++) {
		...
		for (int idx = 0; idx < NUM_REQUESTS; idx++) {
			...
			OFINCCLCHECKGOTO(extNet->regMr((void *)sComm_next,
						       (void *)send_buf[idx],
						       SEND_SIZE,
						       buffer_type,
						       &send_mhandle[idx]),
			...
			while (send_req[idx] == NULL) {
				OFINCCLCHECKGOTO(extNet->isend((void *)sComm_next,
							       (void *)send_buf[idx],
							       SEND_SIZE,
							       tag,
							       send_mhandle[idx],
		...
		for (int idx = 0; idx < NUM_REQUESTS; idx++) {
			...
			NCCL_OFI_TRACE(
				NCCL_NET,
				"Successfully registered receive memory for request %d of rank %d",
				idx,
				rank);
		...
		while (inflight_reqs > 0) {
			...
			for (int idx = 0; idx < NUM_REQUESTS; idx++) {
				...
				if (done) {
					...
					OFINCCLCHECKGOTO(extNet->deregMr((void *)sComm_next,
									 send_mhandle[idx]),
							 res,
							 exit);
				}
			...
			for (int idx = 0; idx < NUM_REQUESTS; idx++) {
				...
				if (done) {
					...
					/* Invoke flush operations unless user has explicitly
					 * disabled it */
					...
					if ((buffer_type == NCCL_PTR_CUDA) &&
						...
						/* Data validation may fail if flush operations are
						 * disabled */
					} else {
					...
					/* Deregister memory handle */
					OFINCCLCHECKGOTO(
						extNet->deregMr((void *)rComm, recv_mhandle[idx]),
						res,
						exit);
				}
			}
