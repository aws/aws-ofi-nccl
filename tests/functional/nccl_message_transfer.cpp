...
// Temporary mutex to test memory allocation serialization hypothesis
static std::mutex memory_alloc_mutex;
...
static inline ncclResult_t test_single_size_transfer(test_nccl_net_t* ext_net,
	...
	if (rank == 0) {
		...
		for (int idx = 0; idx < NUM_REQUESTS; idx++) {
			{
				std::lock_guard<std::mutex> lock(memory_alloc_mutex);
...
public:
	...
	ncclResult_t run(ThreadContext& ctx) override {
		...
		for (size_t dev_idx = 0; dev_idx < lcomms_ptr->size(); dev_idx++) {
			...
			int buffer_type = NCCL_PTR_HOST; // Default to host
			if (gdr_support[dev]) {
