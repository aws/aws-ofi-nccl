...
{
	nccl_net_ofi_schedule_t *schedule = malloc(sizeof(nccl_net_ofi_schedule_t) +
...
int verify_xfer_info(nccl_net_ofi_xfer_info_t *xfer,
		     nccl_net_ofi_xfer_info_t *ref_xfer,
		     int xfer_id)
{
	int ret = ref_xfer->rail_id != xfer->rail_id || ref_xfer->offset != xfer->offset ||
		  ref_xfer->msg_size != xfer->msg_size;

	if (ret) {
		NCCL_OFI_WARN(
			"Expected rail_xfer_infos[%i] ={rail_id = %i, offset = %zu, msg_size = "
			"%zu}, but got {rail_id = %i, offset = %zu, msg_size = %zu}",
			xfer_id,
			ref_xfer->rail_id,
			ref_xfer->offset,
			ref_xfer->msg_size,
...
int verify_schedule(nccl_net_ofi_schedule_t *schedule, nccl_net_ofi_schedule_t *ref_schedule)
{
	...
	for (int info_id = 0; info_id != schedule->num_xfer_infos; ++info_id) {
		ret |= verify_xfer_info(&schedule->rail_xfer_infos[info_id],
					&ref_schedule->rail_xfer_infos[info_id],
					info_id);
	}
...
int test_multiplexing_schedule()
{
	...
	nccl_net_ofi_schedule_t *ref_schedule =
		malloc(sizeof(nccl_net_ofi_schedule_t) + 3 * sizeof(nccl_net_ofi_xfer_info_t));
	if (!ref_schedule) {
		NCCL_OFI_WARN("Could not allocate schedule");
		return -ENOMEM;
	}
...
int test_threshold_scheduler()
{
	...
	nccl_net_ofi_schedule_t *ref_schedule = malloc(
