...
/*
 * @brief 	Recursive function that walks top down (parent to child) from the HWLOC tree's root in DFS fashion.
 		  It performs the following operations;

		  1) Identify NUMA nodes that are actually connected to a Package node, meaning the NUMA node has a physical chip / socket presence.
		  	 This is identified via DFS search observing both NUMA and Package node presence through its DFS path.
			 This means, a NUMA <--> Package connection has been established via parent child relationship.
			 This is the "Base case 1", marked by the comment below.

		  2) From hitting this base case, the recursion bubbles-up the call stack, where we remove one node at a time from this DFS path.
		  	 As we remove one node at a time, we eventually reach the parent node between the NUMA <--> Package DFS path.
		  	 We mark the metadata of this parent node using the NUMA Node's pointer value, which is later referred by cpu xml tagging `write_nccl_topo_rec()`.
			 a) For HWLOC 2.x, the parent node is a Package node, as it resides on the higher level of HWLOC hierchy relative to a NUMA node.
		  	 b) For HWLOC 1.x, the parent node is a NUMA node, as it resides on the higher level of HWLOC hierchy relative to a Package node.
		  	 Since it resides higher on the hierchy, when the node is later referred by `write_nccl_topo_rec()` to write cpu opening xml tag, 
			 the xml tagging algorithm is not expected to traverse further down. All the necessary information resides at the highest level.
 ...
 *
 * @return    Pointer to the most recently observed NUMA node, once both Package and NUMA nodes have been found throughout the DFS path.
		  This value bubbles-up the call stack after hitting Base case 1, eventually reaching the parent node as described above.
 */
static hwloc_obj_t identify_numa_connected_to_package_and_associate_to_top_most_node_rec(hwloc_topology_t topo, hwloc_obj_t node, hwloc_obj_t closest_numa_node, bool found_numa, bool found_package) {
	hwloc_obj_t child = NULL;
	...
	while ((child = hwloc_get_next_child(topo, node, child))) {
		hwloc_obj_t ret = identify_numa_connected_to_package_and_associate_to_top_most_node_rec(topo, child, latest_numa_node, found_numa, found_package);
				  ...
				  struct fi_info *info_list)
{
	...
	identify_numa_connected_to_package_and_associate_to_top_most_node_rec(ofi_topo->topo, obj, NULL, false, false);
...
static int write_nccl_topo_rec(hwloc_topology_t topo, hwloc_obj_t node, FILE *file, int indent, int bridge_depth)
{
	 ...
	 * This means, the Package node object is connected with a NUMA node,
	 * marked via `identify_numa_connected_to_package_and_associate_to_top_most_node_rec()`. */
	 ...
	 * This means, the NUMA node object is connected with a Package node,
	 * marked via `identify_numa_connected_to_package_and_associate_to_top_most_node_rec()`. */
		if ((ret = write_cpu_opening_tag(topo_data->closest_numa_node, file, indent))) {
			return ret;
		}
