...
CUresult (*nccl_net_ofi_cuPointerGetAttribute)(void *data,
...
int nccl_net_ofi_get_cuda_device(void *data, int *dev_id)
{
	...
	CUresult cuda_ret_mem = nccl_net_ofi_cuPointerGetAttribute(&mem_type,
								   ...
								   (CUdeviceptr)data);
	CUresult cuda_ret_dev =
		nccl_net_ofi_cuPointerGetAttribute(&device_ordinal,
	...
	if (mem_type == CU_MEMORYTYPE_DEVICE) {
		...
		ret = -EINVAL;
		NCCL_OFI_WARN(
			"Invalid type of buffer provided. Only device memory is expected for "
			"NCCL_PTR_CUDA type");
	}
