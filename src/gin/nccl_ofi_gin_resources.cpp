...
static inline void get_gin_hints(struct fi_info &hints, struct fi_info *ref_info)
{
	...
	/* Copy domain and fabric fid and name from previously opened net transport.
	   Note the domain and fabric names are required to obtain exactly one result. */
	hints.domain_attr->domain = ref_info->domain_attr->domain;
	hints.fabric_attr->fabric = ref_info->fabric_attr->fabric;
