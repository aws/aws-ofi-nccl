...
static inline struct fi_info *get_gin_info(struct fi_info *info)
{
	...
	/* Start with a copy of the info object from the net transport */
	ofi_info_ptr gin_info = ofi_info_ptr(fi_dupinfo(info));

	/* Set GIN-specific hints */
...
nccl_ofi_gin_mr_handle_t::nccl_ofi_gin_mr_handle_t(nccl_net_ofi_domain_t &domain_arg, uint16_t num_rails, uint64_t mr_key_arg)
