 ...
 * not consistent in mapping BDFs such that the natural Libfabric BDF
 * sorting ends up with this pairing happening naturally.  So this
 * code reorders the provider list to make that happen.  There are two
 * pairs of cards behind each PCIe switch, and we don't actually care
 * which is which, only that the oth device talks to the 0th device in
 * and so on.
 *
 ...
 * pair index, and so on.  This should work on all platforms on
 * roadmap, but feels a little icky because it does not guarantee that
 * the first N providers are all actually sharing resources (but it
 * turns out for the one place where we have this sharing, that
 * doesn't matter).  Assuming we get better at BDF ordering, this
 * should all hopefully become a no-op in the future anyway.
 */
