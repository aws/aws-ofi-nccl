...
ncclResult_t nccl_ofi_tuner_init(size_t nRanks, size_t nNodes, ncclDebugLogger_t logFunction)
{
	...
	if (getenv("NCCL_ALGO") || getenv("NCCL_PROTO")) {
		NCCL_OFI_WARN(
			"The tuner plugin can not be loaded when explicitly choosing an algorithm "
			"or protocol with NCCL_ALGO/NCCL_PROTO");
		return ncclInvalidArgument;
	}

	struct nccl_ofi_tuner_model_params params = {.net_lat = ofi_nccl_tuner_net_latency(),
	...
	NCCL_OFI_TRACE(NCCL_TUNING,
		       "Tuner init: comm with %ld ranks and %ld nodes.",
		       nRanks,
		       nNodes);
	return ncclSuccess;
}

ncclResult_t nccl_ofi_tuner_get_coll_info(ncclFunc_t collType,
					  size_t nBytes,
					  int collNetSupport,
					  int nvlsSupport,
					  int numPipeOps,
					  ...
					  int *nChannels)
{
	...
	for (algo = 0; algo < NCCL_NUM_ALGORITHMS; algo++) {
		...
		for (proto = 0; proto < NCCL_NUM_PROTOCOLS; proto++) {
			...
			cost = nccl_ofi_tuner_compute_cost(collType,
							   algo,
							   proto,
							   numPipeOps,
							   nBytes);
			...
			NCCL_OFI_TRACE(
				NCCL_TUNING,
				"Computed cost for algo %d proto %d pipe %d: cost %.8f µsecs.",
				algo,
				proto,
				numPipeOps,
				cost);
