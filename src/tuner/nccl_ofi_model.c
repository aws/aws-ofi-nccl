...
float nccl_ofi_tuner_compute_cost(ncclFunc_t func, int algo, int proto, int pipe_ops, size_t size)
{
	...
	case ncclFuncAllReduce:
		...
		case NCCL_ALGO_NVLS_TREE:
			...
			bw = NCCL_OFI_MIN(params->intranode_bw,
					  (params->internode_bw * params->num_rails) / 2) *
			     ofi_nccl_tuner_num_channels();
			break;
		...
		case NCCL_ALGO_TREE:
			latency =
				((2 *
				  ((nccl_ofi_tuner_ctx->num_ranks / nccl_ofi_tuner_ctx->num_nodes) -
				   1) *
				  p2p_lat) +
		...
		default:
			NCCL_OFI_TRACE(NCCL_TUNING,
				       "Algorithm %d for collective %d  without a model.",
				       algo,
				       func);
			return -1;
		}
	...
	default:
		NCCL_OFI_TRACE(NCCL_TUNING,
			       "Unsupported collective %d, fallback to NCCL's selection.",
			       func);
		return -1;
	}
