...
ncclResult_t nccl_net_ofi_fini_v6()
{
	...
	if (plugin == NULL) {
		ret = nccl_net_ofi_check_return(ncclSystemError);
	} else {
		delete plugin;
		plugin = NULL;
	}
...
ncclResult_t nccl_net_ofi_init_v6(ncclDebugLogger_t logFunction)
{
	...
	if (plugin != nullptr) {
		return nccl_net_ofi_check_return(ncclSystemError);
	}
...
ncclResult_t nccl_net_ofi_devices_v2(int *num_devices)
{
	...
	if (OFI_UNLIKELY(plugin == NULL)) {
		...
		return nccl_net_ofi_check_return(ncclInvalidArgument);
	}
...
ncclResult_t nccl_net_ofi_get_properties(int dev_id, nccl_ofi_properties_t *ofi_properties)
{
	...
	if (device == NULL) {
		...
		return nccl_net_ofi_check_return(ncclInternalError);
	}
