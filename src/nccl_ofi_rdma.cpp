...
static inline bool has_flush_completed(nccl_net_ofi_rdma_req_t *req)
{
	...
	for (uint16_t rail_id = 0; rail_id < ep->num_rails; rail_id++) {
		uint64_t *val = (uint64_t *)((char *)flush_data->flush_fl_elem->ptr + (cpu_cache_line_size * rail_id));
		if (*val != NCCL_OFI_RDMA_FLUSH_BUFFER_SENTINEL) {
			return false;
		}
	}
       return true;
							...
							const nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	ret = nccl_ofi_freelist_init_mr(cpu_cache_line_size * MAX_NUM_RAILS,
					8, 8, NCCL_OFI_MAX_REQUESTS, NULL, NULL,
					freelist_regmr_host_fn,
					freelist_deregmr_host_fn, domain, 1,
...
#endif
#if HAVE_CUDA
		uint64_t *host_buff_addr = (uint64_t *)((char *)flush_data->flush_fl_elem->ptr + (cpu_cache_line_size * rail_id));
		rc = fi_read(comm_rail->local_ep,
			(void *)host_buff_addr,
			NCCL_OFI_FLUSH_SIZE, desc, comm_rail->local_addr,
