 ...
 *		non-zero, on error
 */
static inline int set_recv_ctrl_completed(nccl_net_ofi_rdma_req_t *req)
{
	assert(req->type == NCCL_OFI_RDMA_RECV);
				 ...
				 nccl_net_ofi_rdma_req_t *req)
{
	...
	uint16_t slot = s_comm->next_msg_seq_num % NCCL_OFI_MAX_REQUESTS;
	...
	for (uint16_t rail_id = 0; rail_id != ep->num_rails; ++rail_id) {
		if (ep->use_long_rkeys) {
			send_data->remote_mr_key[rail_id] = s_comm->ctrl_fifo[slot].long_buff_mr_key[rail_id];
		} else {
			send_data->remote_mr_key[rail_id] = s_comm->ctrl_fifo[slot].short_buff_mr_key[rail_id];
		}
	}
					   ...
					   uint16_t rail_id)
{
	...
	if (comp_flags & FI_SEND) {
		...
		case NCCL_OFI_RDMA_RECV: {
			...
			ret = set_recv_ctrl_completed(req);
			break;
		}
		case NCCL_OFI_RDMA_READ:
		case NCCL_OFI_RDMA_SEND_CLOSE:
				...
				bool recv_completion_optional)
{
	...
	for (; rail_id < r_comm->num_rails; rail_id++) {
		...
		if (ep->use_long_rkeys) {
			r_comm->remote_fifo.ctrl_fifo[slot].long_buff_mr_key[rail_id] = rkey;
		} else {
			...
			r_comm->remote_fifo.ctrl_fifo[slot].short_buff_mr_key[rail_id] = rkey;
		}
...
static void prepare_send_connect_message(nccl_net_ofi_rdma_ep_t *ep, uint32_t local_comm_id,
					 nccl_net_ofi_ctrl_fifo* ctrl_fifo, nccl_net_ofi_rdma_mr_handle_t *ctrl_fifo_mr_handle,
		   ...
		   int trafficClass)
{
	...
	*send_comm = NULL;
	...
	if (s_comm == nullptr) {
		...
		nccl_ofi_rdma_connection_info_t conn_msg;
		prepare_send_connect_message(ep, s_comm->local_comm_id, s_comm->ctrl_fifo, s_comm->ctrl_fifo_mr_handle, &conn_msg);
						    ...
						    nccl_net_ofi_ep_t **base_ep)
{
	...
	ep->ctrl_rx_buff_size = std::max({sizeof(nccl_ofi_rdma_connection_info_t),
