...
static inline int free_eager_copy_req(nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_eager_copy_req");
		return -EINVAL;
	}
...
static int post_rma_read(nccl_net_ofi_rdma_req_t *req)
{
	...
	if (comm_rail == nullptr) {
		NCCL_OFI_WARN("Failed to get recv comm rail in post_rma_read");
		return -EINVAL;
	}
...
static int receive_progress(nccl_net_ofi_rdma_req_t *req, bool add_to_pending)
{
	...
	if (rc == -FI_EAGAIN && add_to_pending) {
		nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
		if (r_comm == nullptr) {
			NCCL_OFI_WARN("Recv comm is null in receive_progress");
			return -EINVAL;
		}
		/* Extract ep */
		nccl_net_ofi_rdma_ep_t *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->ep;
		if (ep == nullptr) {
			NCCL_OFI_WARN("Recv comm ep is null in receive_progress");
			return -EINVAL;
		}
				 ...
				 bool dec_inflight_reqs)
{
	...
	if (s_comm == nullptr) {
		NCCL_OFI_WARN("Send comm is null in free_write_req");
		return -EINVAL;
	}
				...
				bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_read_req");
		return -EINVAL;
	}
	return free_base_req(&r_comm->num_inflight_reqs, r_comm->nccl_ofi_reqs_fl,
			req, dec_inflight_reqs);
					 ...
					 bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_send_comm_t *s_comm =
		(nccl_net_ofi_rdma_send_comm_t *)req->comm;
	if (s_comm == nullptr) {
		NCCL_OFI_WARN("Send comm is null in free_send_req");
		return -EINVAL;
	}
					 ...
					 bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_recv_req");
		return -EINVAL;
	}
	rdma_req_recv_data_t *recv_data = get_recv_data(req);
	nccl_net_ofi_rdma_req_t *recv_segms_req = recv_data->recv_segms_req;
					      ...
					      bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_recv_segms_req");
		return -EINVAL;
	}
					      ...
					      bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_send_close_req");
		return -EINVAL;
	}
					  ...
					  bool dec_inflight_reqs)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm =
		(nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in free_flush_req");
		return -EINVAL;
	}
...
{
	rdma_req_flush_data_t *flush_data = get_flush_data(req);
	nccl_net_ofi_xfer_comm_t *base_comm = req->comm;
...
int nccl_net_ofi_rdma_domain_t::deregMr(nccl_net_ofi_xfer_comm_t *comm,
					nccl_net_ofi_mr_handle_t *mr_handle)
{
	return this->dereg_mr_impl(static_cast<nccl_net_ofi_rdma_mr_handle_t *>(mr_handle));
}

int nccl_net_ofi_rdma_domain_t::dereg_mr_impl(nccl_net_ofi_rdma_mr_handle_t *mr_handle)
{
	if (OFI_UNLIKELY(mr_handle == NULL)) {
		return 0;
	}
...
int nccl_net_ofi_rdma_domain_t::regMr(nccl_net_ofi_xfer_comm_t *comm,
				      nccl_ofi_mr_ckey_ref ckey,
				      int type,
				      void **mhandle)
{
	pthread_wrapper domain_scoped_lock(&this->domain_lock);

	CHECK_DOMAIN_ACTIVE(this, "regMr");

	return this->reg_mr_impl(ckey, type,
				 reinterpret_cast<nccl_net_ofi_rdma_mr_handle_t **>(mhandle));
}


int nccl_net_ofi_rdma_domain_t::reg_mr_impl(nccl_ofi_mr_ckey_ref ckey,
					    int type,
					    nccl_net_ofi_rdma_mr_handle_t **mhandle)
{
	int ret = 0;
						...
						nccl_net_ofi_rdma_mr_handle_t **mhandle)
{
	...
	return this->reg_mr_impl(&ckey, type, mhandle);
}
...
static int freelist_regmr_host_fn(void *domain_void_ptr, void *data, size_t size, void **handle)
{
	...
	if (ret != 0) {
		NCCL_OFI_WARN("Failed call to reg_mr_impl: %d", ret);
...
static int freelist_deregmr_host_fn(void *handle)
{
	...
	int ret = freelist_handle->domain->dereg_mr_impl(freelist_handle->mr_handle);
	if (OFI_UNLIKELY(ret != 0)) {
		NCCL_OFI_WARN("Failed call to dereg_mr_impl");
		return -EIO;
	}
	free(freelist_handle);
	return 0;
...
int nccl_net_ofi_rdma_recv_comm_t::recv(int n, void **buffers,
					size_t *sizes, int *tags,
					nccl_net_ofi_mr_handle_t **mhandles,
					nccl_net_ofi_req_t **base_req)
{
	int ret = 0;
	nccl_net_ofi_rdma_req_t *req = nullptr;
	rdma_req_recv_data_t *recv_data = nullptr;
	nccl_net_ofi_rdma_ep_t *ep_ptr = nullptr;
	nccl_net_ofi_rdma_domain_t *domain = nullptr;
	nccl_net_ofi_rdma_device_t *device = nullptr;
	auto **mr_handles = reinterpret_cast<nccl_net_ofi_rdma_mr_handle_t **>(mhandles);
	uint16_t msg_seq_num = 0;
	bool eager = false;
	int i;
	bool recv_completion_optional = false;

	if (early_completion && *base_req == reinterpret_cast<void *>(NCCL_NET_OPTIONAL_RECV_COMPLETION)) {
	...
	ep_ptr = this->rdma_recv_comm_get_ep();
	assert(ep_ptr != nullptr);

	domain = ep_ptr->rdma_endpoint_get_domain();
	assert(domain != nullptr);

	device = ep_ptr->rdma_endpoint_get_device();
	...
	ret = ep_ptr->process_cq_if_pending();
	if (ret == -EAGAIN) {
	...
	ret = allocate_rdma_recv_req(this, device, this->dev_id, msg_seq_num,
				     buffers[0], sizes[0],
	...
	if (eager) {
		...
		if (rx_buff_data->recv_len == 0) {
			...
			recv_data->eager_copy_req = nullptr;
		} else {
	...
	NCCL_OFI_TRACE_RECV(this->dev_id, this, sizes[0], req, base_req);
	...
	if (eager) {
		if (recv_data->eager_copy_req == nullptr) {
...
int nccl_net_ofi_rdma_domain_t::dealloc_and_dereg_flush_buff()
{
	...
	if (mr_handle) {
		ret = this->dereg_mr_impl(mr_handle);
	}
	if (ret != 0) {
...
static int recv_comm_destroy(nccl_net_ofi_rdma_recv_comm_t *r_comm)
{
	...
	domain->dereg_mr_impl(r_comm->ctrl_mr_handle);
...
static int send_comm_destroy(nccl_net_ofi_rdma_send_comm_t *s_comm)
{
	...
	domain->dereg_mr_impl(s_comm->ctrl_mr_handle);
...
int nccl_net_ofi_rdma_recv_comm_t::close()
{
	...
	if (this->num_inflight_reqs > 0 && this->num_inflight_reqs > this->num_pending_flush_comps) {
		...
		auto *ep_ptr = this->rdma_recv_comm_get_ep();
		ep_ptr->rdma_endpoint_abort();
	}
...
int nccl_net_ofi_rdma_recv_comm_t::flush(int n, void **buffers,
					 ...
					 nccl_net_ofi_req_t **base_req)
{
	...
	auto *domain = ep_ptr->rdma_endpoint_get_domain();
	pthread_wrapper domain_lock(&domain->domain_lock);
	...
	/* Process any pending requests */
	network_busy = false;
	rc = ep_ptr->process_cq_if_pending();
	if (rc == -EAGAIN) {
		/* Network is still busy. */
		network_busy = true;
	} else if (rc != 0) {
	...
	if (!network_busy) {
		...
		/* Add to pending reqs queue */
		nccl_net_ofi_mutex_lock(&ep_ptr->pending_reqs_lock);
		ep_ptr->pending_reqs_queue.push_back(req);
		nccl_net_ofi_mutex_unlock(&ep_ptr->pending_reqs_lock);
		ret = 0;
		NCCL_OFI_TRACE_PENDING_INSERT(req);
	}
...
static inline nccl_net_ofi_rdma_recv_comm_t *calloc_rdma_recv_comm(int num_rails, int num_control_rails)
{
	...
	auto *r_comm = new nccl_net_ofi_rdma_recv_comm_t;
...
int nccl_net_ofi_rdma_recv_comm_t::read(void* dest, size_t size, void* mhandle,
					uint64_t src, uint64_t mr_key,
					nccl_net_ofi_req_t **base_req)
{
	int ret = 0;
	auto *mr_handle = static_cast<nccl_net_ofi_rdma_mr_handle_t *>(mhandle);
	nccl_net_ofi_rdma_req_t *req = nullptr;
	nccl_net_ofi_rdma_ep_t *ep_ptr = nullptr;
	...
	ret = alloc_rdma_read_req(this, ep_ptr, dest,
							...
							const nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	r_comm->num_pending_flush_comps = 0;
	...
	for (uint16_t rail_id = 0; rail_id != num_control_rails; ++rail_id) {
		...
		if (comm_rail == nullptr) {
			NCCL_OFI_WARN("Failed to get recv comm control rail in prepare_recv_comm");
			goto error;
		}
	...
	for (uint16_t rail_id = 0; rail_id != num_rails; ++rail_id) {
		auto *comm_rail = r_comm->rdma_recv_comm_get_rail(rail_id);
		if (comm_rail == nullptr) {
			NCCL_OFI_WARN("Failed to get recv comm rail in prepare_recv_comm");
			goto error;
		}
...
int nccl_net_ofi_rdma_listen_comm_t::accept(nccl_net_ofi_xfer_comm_t **recv_comm)
{
	int ret = 0;

	/* Retrieve and validate endpoint */
	auto *ep_ptr = this->rdma_listen_comm_get_ep();
	assert(ep_ptr != nullptr);

	nccl_net_ofi_rdma_ep_t *r_comm_ep = nullptr;
	if (this->r_comm) {
		r_comm_ep = this->r_comm->rdma_recv_comm_get_ep();
		assert(r_comm_ep != nullptr);
	}
...
int nccl_net_ofi_rdma_listen_comm_t::close()
{
	int ret = 0;

	/* Retrieve and validate endpoint */
	nccl_net_ofi_ep_t *ep_ptr = this->ep;
	assert(ep_ptr != nullptr);
				   ...
				   nccl_net_ofi_listen_comm_t **listen_comm)
{
	...
	/* Build listen_comm */
	l_comm = new nccl_net_ofi_rdma_listen_comm_t;

	/* Initialize listen communicator */
	l_comm->type = NCCL_NET_OFI_LISTEN_COMM;
	l_comm->stage = COMM_CREATE_START;
	l_comm->ep = this;
...
static int post_rma_write(nccl_net_ofi_rdma_req_t *req)
{
	...
	if (s_comm == nullptr) {
		NCCL_OFI_WARN("Send comm is null in post_rma_write");
		return -EINVAL;
	}
	uint16_t rail_id = 0;
	auto *comm_rail = s_comm->rdma_send_comm_get_rail(rail_id);
	if (comm_rail == nullptr) {
		NCCL_OFI_WARN("Failed to get send comm rail in post_rma_write");
		return -EINVAL;
	}
...
static int send_progress(nccl_net_ofi_rdma_req_t *req)
{
	...
	if (req->type == NCCL_OFI_RDMA_SEND) { // Post RDMA write
		nccl_net_ofi_rdma_send_comm_t *s_comm = (nccl_net_ofi_rdma_send_comm_t *)req->comm;
		if (s_comm == nullptr) {
			NCCL_OFI_WARN("Send comm is null in send_progress");
			return -EINVAL;
		}
		...
		if (send_data->eager) {
			...
			if (comm_rail == nullptr) {
				NCCL_OFI_WARN("Failed to get send comm rail in send_progress eager");
				return -EINVAL;
			}
			...
			for (uint16_t rail_it = send_data->xferred_rail_id; rail_it < schedule->num_xfer_infos; rail_it++) {
				...
				nccl_net_ofi_rdma_send_comm_rail_t *comm_rail =
					s_comm->rdma_send_comm_get_rail(xfer_info->rail_id);
				if (comm_rail == nullptr) {
					NCCL_OFI_WARN("Failed to get send comm rail in send_progress non-eager");
					return -EINVAL;
				}
			  ...
			  nccl_net_ofi_rdma_req_t *req)
{
	...
	auto *comm_rail = r_comm->rdma_recv_comm_get_control_rail(rail_id);
	if (comm_rail == nullptr) {
		NCCL_OFI_WARN("Failed to get recv comm control rail in send_ctrl_post");
		return -EINVAL;
	}
...
static int post_rdma_ctrl(nccl_net_ofi_rdma_req_t *req)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in post_rdma_ctrl");
		return -EINVAL;
	}
	nccl_net_ofi_rdma_ep_t *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->ep;
	if (ep == nullptr) {
		NCCL_OFI_WARN("Recv comm ep is null in post_rdma_ctrl");
		return -EINVAL;
	}
	nccl_net_ofi_rdma_domain_t *domain = ep->rdma_endpoint_get_domain();
	assert(domain != NULL);
	...
	auto *comm_rail = r_comm->rdma_recv_comm_get_control_rail(rail_id);
	if (comm_rail == nullptr) {
		NCCL_OFI_WARN("Recv comm rail for rail_id %d is null in post_rdma_ctrl", rail_id);
		return -EINVAL;
	}
...
static int post_close_msg(nccl_net_ofi_rdma_req_t *req)
{
	...
	nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in post_close_msg");
		return -EINVAL;
	}
	rdma_req_send_close_data_t *send_close_data = req_get_send_close_data(req);
...
static int post_eager_copy(nccl_net_ofi_rdma_req_t *req)
{
	nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (r_comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in post_eager_copy");
		return -EINVAL;
	}
	rdma_req_eager_copy_data_t *eager_copy_data = get_eager_copy_data(req);
	rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(eager_copy_data->eager_rx_buff_req);
...
static int post_flush_req(nccl_net_ofi_rdma_req_t *req)
{
 	nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
	if (req->comm == nullptr) {
		NCCL_OFI_WARN("Recv comm is null in post_flush_req");
		return -EINVAL;
	}
	nccl_net_ofi_rdma_ep_t *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->ep;
	if (ep == nullptr) {
		NCCL_OFI_WARN("Recv comm ep is null in post_flush_req");
		return -EINVAL;
	}
	nccl_net_ofi_rdma_domain_t *domain = ep->rdma_endpoint_get_domain();
	rdma_req_flush_data_t *flush_data = get_flush_data(req);
					...
					nccl_net_ofi_req_t **base_req)
{
	...
	ep_ptr = this->rdma_send_comm_get_ep();
	assert(ep_ptr != nullptr);
	...
	if (!have_ctrl && static_cast<ssize_t>(size) <= ep_ptr->eager_send_size && this->num_inflight_writes == 0) {
		eager = true;
	}
	...
	NCCL_OFI_TRACE_SEND(req->dev_id, size, this, msg_seq_num, req, base_req);
...
int nccl_net_ofi_rdma_send_comm_t::close()
{
	...
	if (this->num_inflight_reqs > 0) {
		...
		auto *ep_ptr = this->rdma_send_comm_get_ep();
		ep_ptr->rdma_endpoint_abort();
	} else {
...
static inline nccl_net_ofi_rdma_send_comm_t *calloc_rdma_send_comm(int num_rails, int num_control_rails)
{
	...
	auto *s_comm = new nccl_net_ofi_rdma_send_comm_t;
 ...
 */
static int rma_write_impl(nccl_net_ofi_xfer_comm_t *send_comm, void* src, size_t size, void* desc,
...
int nccl_net_ofi_rdma_send_comm_t::write_inline(void *src, size_t size, uint64_t dest,
						uint64_t mr_key,
						nccl_net_ofi_req_t **base_req)
{
	void *desc = nullptr;
...
int nccl_net_ofi_rdma_ep_t::create_send_comm(nccl_net_ofi_rdma_send_comm_t **s_comm)
{
	...
	*s_comm = NULL;
	...
	ret_s_comm->num_inflight_writes = 0;
				    ...
				    int trafficClass)
{
	...
	*send_comm = nullptr;
	...
	nccl_net_ofi_rdma_send_comm_t *s_comm =
		static_cast<nccl_net_ofi_rdma_send_comm_t *>(comm_state->comm);
