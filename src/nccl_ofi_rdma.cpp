...
static int recv_comm_destroy(nccl_net_ofi_rdma_recv_comm_t *r_comm)
{
	...
	domain->dereg_mr(r_comm->remote_fifo.ctrl_fifo_mr_handle);
...
static int send_comm_destroy(nccl_net_ofi_rdma_send_comm_t *s_comm)
{
	...
	/* Deregister control fifo */
	domain->dereg_mr(s_comm->ctrl_fifo_mr_handle);
							...
							const nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	ret = domain->reg_internal_mr(r_comm->remote_fifo.ctrl_fifo, sizeof(nccl_net_ofi_ctrl_fifo) * NCCL_OFI_MAX_REQUESTS, NCCL_PTR_HOST, &mr_handle);
	if (ret != 0) {
...
int nccl_net_ofi_rdma_ep_t::create_send_comm(nccl_net_ofi_rdma_send_comm_t **s_comm)
{
	...
	*s_comm = NULL;
	...
	/* Allocate fifo */
	ret = domain_ptr->reg_internal_mr(ret_s_comm->ctrl_fifo, sizeof(nccl_net_ofi_ctrl_fifo) * NCCL_OFI_MAX_REQUESTS,
				    ...
				    int trafficClass)
{
	...
	*send_comm = nullptr;
	...
	if (s_comm == nullptr) {
		...
		this->prepare_send_connect_message(s_comm->local_comm_id, s_comm->ctrl_fifo, s_comm->ctrl_fifo_mr_handle, &conn_msg);
