...
{
	nccl_net_ofi_rdma_listen_comm_t *l_comm = (nccl_net_ofi_rdma_listen_comm_t *)device->rdma_device_get_comm(local_comm_id);
							   ...
							   uint32_t local_comm_id)
{
	nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)
		device->rdma_device_get_comm(local_comm_id);
					     ...
					     nccl_net_ofi_rdma_req_t *rx_buff_req)
{
	...
	if (rx_buff_data->recv_len == 0) {
		...
		/* Re-post rx buffer */
		ret = rx_buff_req->check_post_rx_buff_req();
		if (ret != 0) {
	...
	ret = recv_data->eager_copy_req->receive_progress(true);
	if (ret != 0) {
				     ...
				     nccl_net_ofi_rdma_req_t *rx_buff_req, bool eager)
{
	...
	case NCCL_OFI_RDMA_MSG_CONN:
		...
		conn_msg = rx_buff_data->get_rx_connection_msg();
	...
	case NCCL_OFI_RDMA_MSG_CONN_RESP:
		...
		conn_resp_msg = rx_buff_data->get_rx_connection_msg();
		s_comm = device->rdma_device_get_send_comm(conn_resp_msg->remote_comm_id);
	...
	case NCCL_OFI_RDMA_MSG_CLOSE:
		...
		ret = rx_buff_req->handle_close_msg_recv();
	...
	for (comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		if (comp_flags & FI_REMOTE_WRITE) {
			...
			if (comp_flags & FI_SEND) {
				...
				case NCCL_OFI_RDMA_EAGER_COPY: {
					ret = req->set_eager_copy_completed();
					break;
				}
				case NCCL_OFI_RDMA_READ: {
			 ...
			 nccl_net_ofi_req_t **base_req)
{
	...
	ret = recv_data->send_ctrl_req->receive_progress(true);
	if (OFI_UNLIKELY(ret != 0)) {
	...
	for (auto it = r_comm_cleanup_list->begin(); it != r_comm_cleanup_list->end();) {
		...
		if (r_comm->send_close_req == NULL) {
			...
			if (all_ctrl_msgs_delivered) {
				...
				ret = r_comm->send_close_req->receive_progress(true);
				if (ret != 0) {
					goto exit;
				}
			}
