						 ...
						 nccl_net_ofi_rdma_mr_handle_t **mhandle)
{
	...
	*mhandle = NULL;
	...
	for (uint16_t rail_id = 0; rail_id != num_rails; ++rail_id) {
		...
		ret_handle->mr[rail_id] = nccl_ofi_ofiutils_mr_regattr(domain_rail->domain, &mr_attr, regattr_flags);
					 ...
					 uint32_t tclass)
{
	...
	ep_rail->av = nccl_ofi_ofiutils_av_create(domain_rail->domain);
	...
	ep_rail->ofi_ep = nccl_ofi_ofiutils_ep_create(rail_info, domain_rail->domain,
	...
	: nccl_net_ofi_domain_t(device_arg)
{
	...
	for (uint16_t i = 0; i < this->num_rails ; i++) {
		...
		domain_rail->domain = nccl_ofi_ofiutils_domain_create(device_rail->fabric, device_rail->info);
		...
		domain_rail->cq = nccl_ofi_ofiutils_cq_create(domain_rail->domain, &cq_attr);
