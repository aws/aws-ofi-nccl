 ...
 */
int nccl_net_ofi_rdma_recv_comm::eager_copy_to_sub_recv(
				  ...
				  int sub_idx)
{
	...
	if (rx_buff_data->recv_len == 0) {
		...
		if (recv_data->num_recvs > 1) {
			return inc_recv_seg_completion(recv_data, 0);
		} else {
					     ...
					     nccl_net_ofi_rdma_req *rx_buff_req)
{
	...
	if (this->recv_eager_free.empty()) {
		NCCL_OFI_WARN("Receiver eager queue full");
		return -ENOMEM;
	}

	nccl_ofi_dlist_node *free_node = this->recv_eager_free.pop_front();
...
static ssize_t ofi_process_cq_rail(nccl_net_ofi_rdma_device_t *device, nccl_net_ofi_rdma_cq_rail_t *rail)
{
...
int rdma_send_req::post()
{
	...
	if (data->eager) {
		/* Eager send: post with fi_senddata on a single rail. */
		...
		ssize_t rc = fi_senddata(comm_rail->local_ep,
					 (void *)(((uintptr_t)data->buff) + xfer_info->offset),
					 xfer_info->msg_size, desc,
					 data->wdata, comm_rail->remote_addr,
...
int rdma_eager_copy_req::post()
{
	 ...
	 * receiver's destination buffer.  Only used for single-recv eager
	 * paths, so we index recvs[0]. */
	nccl_net_ofi_rdma_recv_comm *r_comm = this->get_recv_comm();
	...
	/* Validate size of data (eager only used for single recv) */
	if (parent_recv_data->recvs[0].dst_len < rx_data->recv_len) {
		NCCL_OFI_TRACE(NCCL_NET, "Recv buffer (%zu) smaller than eager send size (%zu)",
			       parent_recv_data->recvs[0].dst_len, rx_data->recv_len);
		rx_data->recv_len = parent_recv_data->recvs[0].dst_len;
	}
	...
	nccl_net_ofi_rdma_mr_handle_t *dest_mr_handle = parent_recv_data->recvs[0].dest_mr_handle;
	...
	ssize_t rc = fi_read(comm_rail->local_ep, parent_recv_data->recvs[0].dst_buff,
				...
				bool recv_completion_optional)
{
	...
	for (i = 0; i < num_recvs; i++) {
		...
		for (uint16_t rail_id = 0; rail_id < this->num_rails; rail_id++) {
			uint64_t rkey = fi_mr_key(mr_h->mr_data[rail_id].get());
									  ...
									  nccl_net_ofi_req **base_req)
{
	...
	*base_req = (nccl_net_ofi_req *)req;
	...
	if (!this->recv_eager_list.empty()) {
		int drain_ret = this->drain_recv_eager_queue();
		if (drain_ret != 0) {
			ret = drain_ret;
			goto error;
		}
						      ...
						      nccl_net_ofi_rdma_mr_handle_t *&mr_out)
{
	...
	ret = rdma_domain->reg_internal_mr(fb.buffer, system_page_size,
				NCCL_PTR_HOST, bind_ep, &mr_handle);
	if (OFI_UNLIKELY(ret != 0)) {
...
static int send_comm_destroy(nccl_net_ofi_rdma_send_comm *s_comm)
{
	...
	delete s_comm->eager_hdr_fl;
	delete s_comm->eager_hdr_regmr_ctx;
	s_comm->eager_hdr_regmr_ctx = nullptr;
							...
							const nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	ret = domain->reg_internal_mr(r_comm->ctrl_mailbox,
		...
		NCCL_PTR_HOST, endpoint_mr ? l_comm_ep : nullptr, &mr_handle);
	if (ret != 0) {
 ...
 */
bool nccl_net_ofi_rdma_send_comm::drain_sender_eager_queue()
{
	while (this->eager_queue_count > 0) {
		...
		if (this->group_sends_remaining == 0) {
			/* Start a new group */
			this->n_ctrl_received += 1;
			this->group_num_recvs = num_recvs;
			this->group_sends_remaining = num_recvs;
		}
		...
		if (!found) {
			...
			this->eager_queue_tail = (tail + 1) % NCCL_OFI_MAX_EAGER_PENDING;
			this->eager_queue_count++;
			break;
                         			  ...
                         			  nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req **base_req)
{
	...
	if (s_comm->eager_queue_count > 0) {
		have_ctrl = this->drain_sender_eager_queue();
		if (have_ctrl) {
	...
	if (OFI_UNLIKELY(!data_progress_auto && !have_ctrl)) {
		...
		have_ctrl = has_ctrl_msg(s_comm, msg_seq_num, tag, false);
	}
...
int nccl_net_ofi_rdma_ep_t::create_send_comm(nccl_net_ofi_rdma_send_comm **s_comm)
{
	...
	*s_comm = NULL;
	...
	ret_s_comm->eager_hdr_regmr_ctx = new freelist_regmr_ep_ctx_t{domain_ptr, endpoint_mr ? this : nullptr};
	ret_s_comm->eager_hdr_fl = new nccl_ofi_freelist(
		sizeof(nccl_ofi_eager_msg_header_t),
		NCCL_OFI_CTRL_MAILBOX_SIZE, 16, 0,
		NULL, NULL,
		freelist_regmr_ep_ctx_t::regmr_fn, freelist_regmr_ep_ctx_t::deregmr_fn,
		ret_s_comm->eager_hdr_regmr_ctx, NCCL_OFI_EAGER_HEADER_SIZE,
		"Eager Header", true);
