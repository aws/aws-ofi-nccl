...
{
	std::lock_guard<std::mutex> lock(resources.cm_mutex);
	size_t n_removed = resources.callback_map.erase(listener_id);
	if (n_removed != 1) {
		NCCL_OFI_WARN("Failed to remove listener id: %lu", listener_id);
		assert(false);
	}
}
...
void nccl_ofi_cm_receiver::set_conn_resp_msg_data(const void *data, size_t size)
{
	...
	memcpy(&conn_resp_msg + 1, data, size);
}
	...
	send_connector_id(resources.get_next_connector_id())
{
	...
	memcpy(&conn_msg + 1, transport_connect_msg, conn_msg_size);
}

nccl_ofi_cm_send_connector::~nccl_ofi_cm_send_connector()
{
	std::lock_guard<std::mutex> lock(resources.cm_mutex);
	size_t n_removed = resources.callback_map.erase(send_connector_id);
	if (n_removed != 1) {
		NCCL_OFI_WARN("Failed to remove connector id: %lu", send_connector_id);
		assert(false);
	}
}
