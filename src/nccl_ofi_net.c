...
int nccl_net_ofi_create_plugin(nccl_net_ofi_plugin_t **plugin_p)
{
	...
	ret = base_ep->release_ep(base_ep, false);
	if (ret != 0) {
		goto exit;
	}
...
int nccl_net_ofi_device_release_all_domain_and_ep(nccl_net_ofi_device_t *device)
{
	...
	HASH_ITER(hh, device->domain_table, domain, domain_tmp) {
		...
		ret = domain->release(domain, true);
...
static int nccl_net_ofi_domain_release(nccl_net_ofi_domain_t *domain, bool skip_device_lock)
{
	int ret = 0;
	nccl_net_ofi_device_t *device;
	...
	if (HASH_COUNT(domain->endpoint_table) == 0 || skip_device_lock) {
		// Force cleanup when skip_device_lock is set.
		if (!skip_device_lock) {
			nccl_net_ofi_mutex_lock(&device->device_lock);
		}
...
int nccl_net_ofi_endpoint_release(nccl_net_ofi_ep_t *ep, bool skip_lock)
{
	...
	if (ep->ref_cnt == 0 || skip_lock) {
		HASH_DEL(domain->endpoint_table, ep);

		if (skip_lock && ep->ref_cnt > 0) {
...
cleanup:
	...
	if (!skip_lock) {
		...
		if (ret == 0) {
			ret = domain->release(domain, false);
		}
	}
