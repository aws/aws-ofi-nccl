					    ...
					    size_t system_page_size)
{
	...
	ret_cache->slots = calloc(init_num_entries, sizeof(ret_cache->slots));
	if (!ret_cache->slots) {
...
static inline void compute_page_address(uintptr_t addr, size_t size, uintptr_t system_page_size,
					uintptr_t *page_addr, size_t *pages)
{
				     ...
				     size_t size)
{
	...
	compute_page_address((uintptr_t)addr, size, (uintptr_t)cache->system_page_size, &page_addr, &pages);
	...
	for (int slot = 0;;slot++) {
		...
		if (slot == cache->used || page_addr < cache->slots[slot]->addr) {
			...
			return NULL;
                           ...
                           ((page_addr - cache->slots[slot]->addr)/cache->system_page_size
				   ...
				   void *handle)
{
	...
	for (int slot = 0;;slot++) {
		...
		if (slot == cache->used || page_addr < cache->slots[slot]->addr) {
			...
			assert(cache->slots);
			memmove(cache->slots+slot+1, cache->slots+slot, (cache->used
				- slot) * sizeof(nccl_ofi_reg_entry_t*));
...
int nccl_ofi_mr_cache_del_entry(nccl_ofi_mr_cache_t *cache, void *handle)
{
	...
	memmove(cache->slots+slot, cache->slots+slot+1, (cache->used-slot-1)
		*sizeof(nccl_ofi_mr_cache_t*));
	--cache->used;
