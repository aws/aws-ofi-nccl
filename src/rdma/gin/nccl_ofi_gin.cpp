...
int nccl_ofi_rdma_gin_put_comm::regMrSymDmaBufGdaki(nccl_ofi_mr_ckey_ref ckey, void *data_ptr, size_t size,
				      int type, uint64_t mrFlags, nccl_ofi_gin_symm_mr_handle_t **mr_handle_out)
{
	auto &gin_ep = resources.get_ep();
	std::lock_guard scoped_ep_lock(gin_ep.ep_lock);

	/* GDAKI path: same shared registration (including dedup), but no GDRCopy —
	   signals are delivered GPU-side via HW counters, and GDRCopy cannot pin
	   multi-segment VMM allocations. gdr_handle is left nullptr. */
	nccl_ofi_rdma_gin_symm_mr_handle *mr_handle = nullptr;
	int ret = regMrSymDmaBufCommon(ckey, data_ptr, size, type, &mr_handle);
	if (ret != 0) {
		return ret;
