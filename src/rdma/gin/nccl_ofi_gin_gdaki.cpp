...
/* v2 sibling of populate_dev_handle: the same handle minus the device-visible
 * CQs and locks, plus the host-published completion state FlushAsync and Wait
 * read. */
static void populate_dev_handle_v2(nccl_ofi_gin_gdaki_dev_handle_v2 &h,
				   const nccl_ofi_gin_gdaki_context *ctx,
				   int ctx_id,
				   int nranks, int rank)
{
						     ...
						     ncclNetDeviceHandle_v11_t **devHandle)
{
	...
	try {
		...
		for (int ctx_id = 0; ctx_id < nContexts; ctx_id++) {
			...
			if (backend_version == NCCL_OFI_GDAKI_BACKEND_VERSION_1) {
				build_handle_array(*ctx->d_counter_handles[ctx_id],
						   config->nCounters,
						   [&](int i) {
							   return ctx->sc_endpoints[ctx_id][i]
								   ->counter_dev_handle.dev;
						   });
				build_handle_array(*ctx->d_signal_handles[ctx_id],
						   config->nSignals,
						   [&](int i) {
							   return ctx->sc_endpoints[ctx_id][i]
								   ->signal_dev_handle.dev;
						   });
			} else {
		...
		if (backend_version == NCCL_OFI_GDAKI_BACKEND_VERSION_1) {
			...
			for (int ctx_id = 0; ctx_id < nContexts; ctx_id++) {
				populate_dev_handle_v2(ctx->dev_handles_v2.host[ctx_id],
						    ctx.get(),
						    ctx_id,
						    nranks,
						    rank);
