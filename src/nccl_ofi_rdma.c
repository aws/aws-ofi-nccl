...
static inline int handle_flush_comp(nccl_net_ofi_rdma_req_t *req)
{
	...
	ret = inc_req_completion(req, 0, ep->num_rails);
...
static int rdma_comm_alloc_frush_req(nccl_net_ofi_rdma_recv_comm_t *r_comm,
					void *buff,
					nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle,
					nccl_net_ofi_rdma_req_t **ret_req)
{
				   ...
				   nccl_net_ofi_req_t **base_req)
{
	...
	ret = rdma_comm_alloc_frush_req(r_comm, buffers[flush_n], mr_handles[flush_n], &req);
	if (OFI_UNLIKELY(ret != 0)) {
		goto error;
	}
	...
	*found_multiple_rails = false;
	...
	if (cpu_cache_line_size < 0) {
		NCCL_OFI_INFO(NCCL_INIT | NCCL_NET,
			      ...
			      NCCL_OFI_CPU_CACHE_LINE_SIZE);
		cpu_cache_line_size = NCCL_OFI_CPU_CACHE_LINE_SIZE;
	}
