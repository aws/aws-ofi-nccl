			     ...
			     nccl_net_ofi_listen_comm_t **listen_comm)
{
	...
	memcpy(handle->ep_name, ep->control_rail.local_ep_name,
	       sizeof(ep->control_rail.local_ep_name));
			      ...
			      nccl_net_ofi_ep_rail_t *ep_rail)
{
	...
	/* if this is a control rail, the rail_id will be 0; it's
	   really the railid of the domain, not the endpoint. This
	   will need to be fixed to support FI_MR_ENDPOINT. */
			      ...
			      nccl_ofi_rdma_connection_info_t* conn_msg)
{
	...
	memcpy(conn_msg->control_ep_name.ep_name, ep->control_rail.local_ep_name,
	       sizeof(ep->control_rail.local_ep_name));
