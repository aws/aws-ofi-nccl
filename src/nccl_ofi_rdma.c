...
static int recv_comm_destroy(nccl_net_ofi_rdma_recv_comm_t *r_comm)
{
	...
	ret = ep->base.release_ep(&ep->base, false);
...
static int nccl_net_ofi_rdma_endpoint_release(nccl_net_ofi_ep_t *base_ep, bool skip_lock)
{
	int ret = 0;
	nccl_net_ofi_rdma_ep_t *ep = NULL;
	...
	if (ep->is_endpoint_per_communicator_ep) {
		...
		if ((--ep->base.ref_cnt) == 0 || skip_lock) {
			if (skip_lock && ep->base.ref_cnt > 0) {
 ...
 unlock:
		...
		ret = nccl_net_ofi_endpoint_release(&ep->base, skip_lock);
	}
...
error:
	if (ret != 0) {
		ep->base.release_ep(&(ep->base), false);
	}
...
error:
	if (ret != 0) {
		domain->base.release(&(domain->base), false);
		domain = NULL;
	}
