				   ...
				   nccl_net_ofi_sendrecv_req_t *req)
{
	...
	auto *freelist_handle = 
		reinterpret_cast<sendrecv_freelist_mr_handle_t *>(l_comm->conn_info->mr_handle);
	void *desc = fi_mr_desc(freelist_handle->mr_handle->mr);
					...
					nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	auto* ret_handle = new nccl_net_ofi_sendrecv_mr_handle_t{0, nullptr};
...
exit:
	if (ret_handle) {
		if (ret_handle->mr) {
			/* Deregister the memory region if it was registered */
			fi_close(&ret_handle->mr->fid);
		}
		/* TODO: 0 is also a valid key to be allocated due to today's key range
		 * being [0, size-1]. This is a edge case that we may leak this one key,
		 * if very unfortunately fi_mr_regattr failed and 0 was allocated as key.
		 * But it is already way better that no any clean up before.
		 * Will need to come back patch this by probably reserving index 0 in the
		 * idpool
		 */
		if (nccl_ofi_idpool_active(key_pool) && ret_handle->mr_key != 0) {
			nccl_ofi_idpool_free_id(key_pool, ret_handle->mr_key);
		}
						       ...
						       nccl_net_ofi_sendrecv_req_t *req)
{
	...
	nccl_ofi_connection_info_t *conn_info = (nccl_ofi_connection_info_t *)s_comm->conn_info->ptr;
	auto* freelist_handle = static_cast<sendrecv_freelist_mr_handle_t *>(s_comm->conn_info->mr_handle);
	void *desc = fi_mr_desc(freelist_handle->mr_handle->mr);
