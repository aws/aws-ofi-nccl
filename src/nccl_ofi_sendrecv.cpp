...
nccl_net_ofi_sendrecv_domain_t::~nccl_net_ofi_sendrecv_domain_t()
{
	...
	if (ret != 0) {
		NCCL_OFI_WARN("RDMA sendrecv domain destructor failed");
	}
}
...
static nccl_net_ofi_domain_t *nccl_net_ofi_sendrecv_device_create_domain(nccl_net_ofi_device_t *base_device, long creating_thread_id)
{
