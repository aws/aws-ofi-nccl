 ...
 *		non-zero on error
 */
static int sendrecv_mr_buffers_internal_register(nccl_net_ofi_sendrecv_domain_t *domain, struct fid_ep *ep,
					int dev_id,
...
static int sendrecv_mr_base_register(nccl_net_ofi_sendrecv_domain_t *domain, struct fid_ep *ep,
				     int dev_id,
...
{
	return this->dereg_mr_impl(static_cast<nccl_net_ofi_sendrecv_mr_handle_t *>(base_mr_handle));
}


int nccl_net_ofi_sendrecv_domain_t::dereg_mr_impl(nccl_net_ofi_sendrecv_mr_handle_t *mr_handle)
{
	int ret = 0;
						...
						nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	ret = sendrecv_mr_base_register(this, ep->ofi_ep, dev_id, ckey, type,
					&ret_handle);
	...
	if (this->mr_cache) {
		...
		if (OFI_UNLIKELY(ret != 0)) {
			 ...
			 */
			if (this->dereg_mr_impl(ret_handle) != 0) {
					     ...
					     nccl_net_ofi_req_t **base_req)
{
	...
	if (this->flush_buff.mr_handle != nullptr) {
		...
		flush_mr_desc = fi_mr_desc(this->flush_buff.mr_handle->mr);
	}
