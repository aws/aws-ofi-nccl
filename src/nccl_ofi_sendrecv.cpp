					...
					nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	ret_handle->mr = nccl_ofi_ofiutils_mr_regattr(domain, 
						      &mr_attr,
						      regattr_flags);
	...
	if (endpoint_mr) {
		ret = fi_mr_bind(ret_handle->mr.get(), &ep->fid, 0);
		if (OFI_UNLIKELY(ret != 0)) {
	...
	: nccl_net_ofi_ep_t(domain_arg)
{
	...
	this->ofi_ep = nccl_ofi_ofiutils_ep_create(device->info, ofi_domain, this->av.get(),
						   domain_arg->cq.get());
	...
	: nccl_net_ofi_domain_t(device_arg)
{
	...
	this->domain = nccl_ofi_ofiutils_domain_create(device_arg->fabric.get(), device_arg->info);
	...
	this->cq = nccl_ofi_ofiutils_cq_create(this->domain.get(), &cq_attr);
