...
This release has been tested with [Libfabric v2.6.0amzn1.0](https://github.com/aws/libfabric/tree/v2.6.0amzn1.0). The plugin requires at least [Libfabric v1.11.0](https://github.com/ofiwg/libfabric/releases/tag/v1.11.0), and compiling AWS-specific support requires at least [Libfabric v1.22.0](https://github.com/ofiwg/libfabric/releases/tag/v1.22.0). GPU-Initiated Networking (GIN) with the kernel backend requires at least [Libfabric v2.6.0](https://github.com/ofiwg/libfabric/releases/tag/v2.6.0).

### New Features:

- **GPU-Initiated Networking (GIN)**: Added support for NCCL device-side networking on EFA. GIN is opt-in and enabled through NCCL, and operates in one of two modes:
  - **Host-proxy mode**: A CPU proxy thread issues network operations on the GPU's behalf.
  - **Kernel backend (EFA GDA)**: NCCL issues work queue entries (WQEs) and rings the NIC doorbell directly from the GPU without any CPU intervention. The plugin automatically uses this mode where supported.

  **Requirements for GIN:**
  ...
  - Kernel backend (EFA GDA): additionally requires [Libfabric 2.6.0](https://github.com/ofiwg/libfabric/releases/tag/v2.6.0)+ and a supported EFA instance type

### Bug Fixes and Improvements:

- For host-proxy GIN, the plugin now coalesces NIC doorbells across a burst of small operations to the same peer, ringing one doorbell per batch instead of one per operation. Large messages continue to stripe across all rails.
- Switched to the relaxed CUDA stream capture mode around CUDA memory allocation, free, and mapping APIs to fix failures when NCCL establishes connections during CUDA graph capture.
- Fixed a start-up failure ("invalid topo group size") by using all available NICs when no NICs are visible at the same PCI level as the accelerator.
- Fixed an occasional hang during NCCL multi-receive communication in small messages/eager mode when using the RDMA transport.
- Fixed a memory leak of the RDMA receive-buffer registration context that recurred on every communicator teardown and could exhaust host memory.
- Added chunk-size tuning for AllGather PAT/Simple and Tree/LL128 on AWS P5en instances, improving performance for medium and large messages.
- On AWS, error completions caused by an EC2 Security Group misconfiguration now trigger a clear warning identifying the likely cause.

### Known Limitations:

- The kernel backend (EFA GDA) is opt-in via `NCCL_GIN_TYPE=5`. When it is enabled, the following limitations apply:
  - Only a fixed signal count per NIC is supported, based on EFA hardware completion counters. When the counters are exhausted, allocation fails and GIN initialization does not complete. No workaround is available in this release.
  - Only Put and signal operations are supported — no get or flush, and indexed signals only (not strong or VA signals).

# v1.20.0 (2026-06)

The 1.20.0 release series has been tested with [NCCL v2.28.9-1](https://github.com/NVIDIA/nccl/releases/tag/v2.28.9-1), [NCCL v2.29.7-1](https://github.com/NVIDIA/nccl/releases/tag/v2.29.7-1), and [NCCL v2.30.4-1](https://github.com/NVIDIA/nccl/releases/tag/v2.30.4-1) while maintaining backward compatibility with older NCCL versions ([NCCL v2.17.1](https://github.com/NVIDIA/nccl/releases/tag/v2.17.1-1) and later).

This release has been tested with [Libfabric v2.4.0amzn5.0](https://github.com/aws/libfabric/tree/v2.4.0amzn5.0). The plugin requires at least [Libfabric v1.11.0](https://github.com/ofiwg/libfabric/releases/tag/v1.11.0), and compiling AWS-specific support requires at least [Libfabric v1.22.0](https://github.com/ofiwg/libfabric/releases/tag/v1.22.0).

### New Features:

- **Multi-receive support**: Added support for NCCL multi-receive, which improves alltoall performance. This optimization is enabled automatically and requires no user configuration changes. In previous releases, this feature required setting `NCCL_OFI_EAGER_MAX_SIZE=-1` (which disabled eager sends); this restriction has been removed.
- **NCCL v2.30 support**: Added support for the NCCL net v12 plugin interface introduced in NCCL v2.30. Older NCCL versions (v2.17.1 and later) continue to work via backward-compatible plugin interfaces.
- **NGC container compatibility**: Added a build option (`--enable-ngc-v1-compat`) to produce plugin libraries compatible with NVIDIA NGC containers that expect the v1 plugin naming convention.
...
- Fixed tuner segfault when `NCCL_ALGO` or `NCCL_PROTO` is explicitly set
- Removed tuner v1 plugin support
...
|---|---|---|
| `NCCL_OFI_EAGER_MAX_SIZE` | **Changed behavior** | Multi-receive no longer requires `-1` to opt in. Eager sends are now auto-detected based on libfabric provider capability (`mixed_hmem_iov`). Default remains `8192` (eager buffer size in bytes). |
...
-Implemented multirecv feature when eager protocol is disabled

# v1.19.0 (2026-04)

The 1.19.0 release series has been tested with [NCCL v2.27.3-1](https://github.com/NVIDIA/nccl/releases/tag/v2.27.3-1), [NCCL v2.28.7-1](https://github.com/NVIDIA/nccl/releases/tag/v2.28.7-1), and [NCCL v2.29.7-1](https://github.com/NVIDIA/nccl/releases/tag/v2.29.7-1) while maintaining backward compatibility with older NCCL versions ([NCCL v2.17.1](https://github.com/NVIDIA/nccl/releases/tag/v2.17.1-1) and later).

This release has been tested with [Libfabric v2.5.0](https://github.com/aws/libfabric/releases/tag/v2.5.0amzn1.0). The plugin requires at least [Libfabric v1.11.0](https://github.com/ofiwg/libfabric/releases/tag/v1.11.0), and compiling AWS-specific support requires at least [Libfabric v1.22.0](https://github.com/ofiwg/libfabric/releases/tag/v1.22.0).
...
The 1.18.0 release series supports [NCCL v2.29.2-1](https://github.com/NVIDIA/nccl/releases/tag/v2.29.2-1) while maintaining backward compatibility with older NCCL versions ([NCCL v2.17.1](https://github.com/NVIDIA/nccl/releases/tag/v2.17.1-1) and later).

With this release, building with platform-aws requires [Libfabric v1.22.0amzn4.0](https://github.com/aws/libfabric/commits/v1.22.0amzn4.0) or greater.
...
The 1.17.3 release series supports [NCCL v2.28.7-1](https://github.com/NVIDIA/nccl/releases/tag/v2.28.7-1) while maintaining backward compatibility with older NCCL versions ([NCCL v2.17.1](https://github.com/NVIDIA/nccl/releases/tag/v2.17.1-1) and later).
...
- Fixed a memory leak that can result in running out of host memory for long-running jobs.

# v1.17.2 (2025-11)

The 1.17.2 release series supports [NCCL v2.28.7-1](https://github.com/NVIDIA/nccl/releases/tag/v2.28.7-1) while maintaining backward compatibility with older NCCL versions ([NCCL v2.17.1](https://github.com/NVIDIA/nccl/releases/tag/v2.17.1-1) and later).
...
- **Control-over-Write Protocol**: Moved away from control messages being sent over send/recv to a mailbox paradigm in RDMA for increased performance
- **Enhanced Tuner Profiles**: Updated tuner regions for P5en and P6 platforms to optimize for control-over-write and NCCL 2.28 channel count changes for AWS platform
- **RDMA Flush Optimization**: Improved flush performance for better throughput
- Updated CUDA APIs to work with CUDA 13.0
- Fixed deadlock in memory registration function
- Updated v1 and v2 of plugin tuner to fallback on nccl tuner when algorithm or protocol is explicitly set
