...
AC_DEFUN([CHECK_PKG_NCCL], [
  ...
  AS_IF([test -n "${with_nccl}"], [NCCL_NET_OFI_DISTCHCK_CONFIGURE_FLAGS="$NCCL_NET_OFI_DISTCHCK_CONFIGURE_FLAGS --with-nccl=${with_nccl}"])

  AS_IF([test -z "${with_nccl}" -o "${with_nccl}" = "yes"],
        [],
        [test "${with_nccl}" = "no"],
        [check_pkg_found=no],
        [AS_IF([test -d ${with_nccl}/lib64], [check_pkg_libdir="lib64"], [check_pkg_libdir="lib"])
  ...
  AS_IF([test "${check_pkg_found}" = "yes"],
        [AC_CHECK_HEADERS([nccl.h], [], [check_pkg_found=no])])
